...
namespace FurnApp_API.Controllers
{
    ...
    public class CategoriesController : ControllerBase
    {
        ...
        {
            var command = new CategoryUpdateCommand { CategoryId = id, CategoryName = CategoryName };
            var response = await _mediator.Send(command);
        ...
        {
            var command = new CategoryDeleteCommand { CategoryId = id };
            var response = await _mediator.Send(command);
        ...
        {
            var command = new GetCategoryQueries();
            var response = await _mediator.Send(command);
            if (response.Success)
            {
                return Ok(response);
